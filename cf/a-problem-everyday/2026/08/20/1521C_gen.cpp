...
int sol() {
    ...
    string s;
    while (getline(cin, s)) {
        if (++ask_cnt > n*3/2+30) break;
        if (s[0] == '!') {
            auto p = parse(s.substr(1));
            bool ok = true;
            if (p.size() != a.size()) ok = false;
            for (int i=0; i<p.size(); i++) {
                if (a[i] != p[i]) ok = false;
            }
            if (!ok) {
                cout << a << " you:" << p << endl;
                cout << "use:" << ask_cnt << endl;
                exit(1);
            }
            return 0;
        }
        if (s[0] == '?') {
            auto p = parse(s.substr(1));
            int t = p[0], i = p[1]-1, j = p[2]-1, x = p[3];
            if (t == 1) {
                cout << max(min(a[i],x), min(a[j],x+1)) << endl;
            } else {
                cout << min(max(a[i],x), max(a[j],x+1)) << endl;
            }
        }
    }
    cout << "ask to much" << endl;
    exit(1);
}
