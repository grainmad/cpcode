...
double get_score() {
	...
	int cnt0=0, cnt1=0;
	int h = area3.size() ? car[area3[0]].drive : 0;
	for (int i=0; i<area3.size(); i++) {
	...
	double final_score = fuel_score+drive_score;
	return final_score;
}

/*
我们认为送车横车机送入 返回道 还是 PBS-总装接车口 对最后的分数影响最大。
例如
最近出现了混动，非混动，非混动，此时出一台非混动就要扣分。
最近出现四驱车型与两驱车型倾向1:1出车序列，此时再出一台与最近相同的车型，则会扣分。
我们觉得需要一个启发函数，
如果当前出车扣分多则减小出车概率

大概的思路是
先计算当前已经出车的序列的对于燃油和驱动扣的分x
再计算当前在横车机上的车也加入出车序列后的对于燃油和驱动扣的分y。
实际上无论怎么出车也不会加分，所以y>=x
若y-x 越大则 进入PBS-总装接车口 概率越小。
*/

bool is_out(int car_id) {
	// cout << "car_id: " << car_id << "curtime: " << curtime << "\n";
	double x = get_score();
	area3.push_back(car_id);
	double y = get_score();
	
	area3.pop_back();
	double rdval = u(e);
	double thre = exp(0.1*(x-y));
	// cout << "x: " << x << " y: " << y << " dif: " << x-y << "\n";
	// cout << "rdval " << rdval << " thre: " << thre << "\n";
	return rdval <= thre;//生成的0到1的随机小数数 小于e^(x-y) 则出车
}
		...
		switch (area1.status) {
			case 0: {
				...
				if (lane[4][0].id) {//返回道10 有车
					area1 = {1, 0, 3, 4, curtime+3};
				} else if (area0.size()) { //涂装-PBS出车口 有车
					area1 = {1, 0, 3, 3, curtime};
				} else return ;//空闲且无车可接，保持空闲
				break;
			}
			case 1: {
				//当为接车时且状态到期时
	...
	while (area2.status == 0 || area2.status_deadline == curtime) {
		switch (area2.status) {
			case 0: {
				...
				// 若队列为空，直接结束行动。
				// 否则取出队首车辆，修改状态为接车，修改横车机目标车道为队首车辆车道，设置状态终止时间
				int choselane=-1, mx = INF;
				for (int i:{0,1,2,3,5,6}) {
					if (lane[i].back().id && mx > lane[i].back().num) {
						mx = lane[i].back().num;
						choselane = i;
					}
				}
				if (choselane == -1) return ;//无车可选
				area2 = {1, 0, 3, choselane, abs(choselane-3)*3+curtime};
				break;
			}
			case 1: {
				// 		当为接车时且状态到期时
				...
				else next_place = is_out(choseCar)?3:4;
